...
$('.btn-ets').click(function(){
    $('.modal-ets button[type=submit]').text("Ajouter")
    $('input, textarea, select').val("")
    $('#logo').attr('src','')
    $('.modal-ets .modal-title').text("Ajouter un etablissement")
    $('.modal-ets').modal({backdrop:'static',keyboard:false})
})


// Assure-toi que ta DataTable est initialisée avant
var table = $('.info-ets').DataTable();
...
$('#ets').on('submit', async function (e) {
    ...
    try {
        const response = await fetch(
            'http://gusto/api-commande/routes/etablissement.php',
            {
                method: 'POST',
                headers: {
        ...
        if (result.success) {
            ...
            // ⚡️ Ajout instantané dans DataTable
            table.row.add(result.data).draw(false); // result.data doit être un array de valeurs
